...
const uint32_t LOG_INTERVAL_USEC = 10000;  //=10ms 10000
//------------------------------------------------------------------------------
...
const uint32_t FILE_BLOCK_COUNT = 256000; //=125MB
//
...
void setup(void) {
  ...
  //   }
      //If the logger was powered down without stopping then reover the data
      recoverTmpFile();
  }
}
//------------------------------------------------------------------------------
void loop(void) {
  ...
  Serial.println(F("c - convert bin to csv"));
  while(!Serial.available()) {
    SysCall::yield();
  }
